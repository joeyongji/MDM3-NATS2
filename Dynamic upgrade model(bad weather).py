...
# =========================
csv_path = "/Users/ivy/Desktop/windfield/windfield_20250725.csv"
...
def generate_spherical_mask_index(Nx, Ny, Nz):
  ...
  # --- 球体 1（大，靠西，较低） ---
    ...
    for iz in range(Nz):
        for iy in range(Ny):
            for ix in range(Nx):
                if (ix - cx1)**2 + (iy - cy1)**2 + (iz - cz1)**2 <= r1**2:
                    mask[iz, iy, ix] = True
                if (ix - cx2)**2 + (iy - cy2)**2 + (iz - cz2)**2 <= r2**2:
                    mask[iz, iy, ix] = True
                if (ix - cx3)**2 + (iy - cy3)**2 + (iz - cz3)**2 <= r3**2:
                    mask[iz, iy, ix] = True
                if (ix - cx4)**2 + (iy - cy4)**2 + (iz - cz4)**2 <= r4**2:
                    mask[iz, iy, ix] = True
...
mappable.set_array([])
cb_quiv = plt.colorbar(mappable, ax=ax, fraction=0.03, pad=0.08, shrink=0.8)
cb_quiv.set_label(
