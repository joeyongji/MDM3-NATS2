...
lat_min, lat_max = -80, 60.0
alt_min, alt_max = 0, 38000.0
...
lon_vals, lat_vals, alt_vals, masks_interp = ew.build_interpolated_mask(output_dir, nat, days)
masks_for_day = {k: v for k, v in masks_interp.items() if k.startswith(f"{day}_")}
blocked = masks_for_day[f"{day}_{time}"]
